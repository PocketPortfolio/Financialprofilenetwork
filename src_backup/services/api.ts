...
export async function getPriceHealth(): Promise<PriceHealthResponse> {
  const r = await fetch(`/api/health-price`, { cache: 'no-store' });
  if (!r.ok) return { providers: [] };
  return r.json() as Promise<PriceHealthResponse>;
}
