...
- **Template (`lib/stack-reveal/email-templates.ts`):**  
  Logo source is chosen at module load by `getLogoSrc()`: if `public/brand/pp-monogram.png` exists, it is read and used as `data:image/png;base64,...`; otherwise fallback to `https://www.pocketportfolio.app/brand/pp-monogram.png`.  
  Header: `<img src="${LOGO_SRC}" alt="Pocket Portfolio" width="40" height="40" ... />`.
...
- **Asset:**  
  `public/brand/pp-monogram.png` is generated by `scripts/generate-logo-png.mjs` (light/white 80×80 PNG, ~5.7KB). Production URL serves this file after deploy.
...
1. **Baked background:** `scripts/generate-logo-png.mjs` now produces a PNG with **solid #0d2818 (dark green) background** and a **white mark** in the center. Contrast is inside the image file, so it survives CSS stripping.
2. **Hosted URL only:** `lib/stack-reveal/email-templates.ts` uses only `LOGO_URL = \`${BASE_URL}/brand/pp-monogram.png?v=2\``. No CID, no Data URI, no local file read. Query param `?v=2` busts Gmail image cache.
3. **Img styling:** Added `border:0;outline:none;text-decoration:none` for Outlook.
4. **No logo attachments:** `lib/stack-reveal/resend.ts` sends no attachments for the logo.

**Verification before deploy:** In an incognito window, open `https://www.pocketportfolio.app/brand/pp-monogram.png`. You should see dark green with a white center block. After deploy, send a test email and confirm the logo renders in Gmail and Outlook.
...
- Code: `lib/stack-reveal/email-templates.ts`, `lib/stack-reveal/resend.ts`, `scripts/generate-logo-png.mjs`, `public/brand/pp-monogram.png`
