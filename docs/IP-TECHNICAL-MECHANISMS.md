...
- **Heuristic vs LLM:** The client runs a heuristic mapper first (synonym-based + optional numeric validation). If confidence is below a threshold and the feature flag is set, the client may request a mapping from the cloud (e.g. `POST /api/ai/map-csv` with `headers` and a small `sampleRows` array). The full CSV is never sent. The server endpoint accepts that request and returns a mapping; the API contract and client flow support LLM-based mapping (constructive reduction to practice).
- **Current server implementation:** When `ENABLE_LLM_IMPORT=true`, `app/api/ai/map-csv/route.ts` tries **Gemini** (`gemini-1.5-flash` via `GOOGLE_GENERATIVE_AI_API_KEY`) first, then **OpenAI** (`generateObject` / `gpt-4o-mini` via `OPENAI_API_KEY`), then falls back to synonym heuristic mapping. Mapping values are validated against the request headers (exact header strings only). The route is unauthenticated so guest Smart Import can call it; cost control is the feature flag plus the truncated payload. When the flag is off, the route returns 403. The patent-relevant behavior remains truncated payload (headers + limited rows) for schema resolution and the full parse remaining local.
