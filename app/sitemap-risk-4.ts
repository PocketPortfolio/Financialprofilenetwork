/**
 * Sitemap: Risk Pages (Part 4)
 * fourth sixteenth of risk pages
...
export default async function sitemapRisk4(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    if (!Array.isArray(tickers)) {
      console.error('[Sitemap Risk-4] getAllTickers() did not return an array:', typeof tickers);
      return [];
    }
    
    if (tickers.length === 0) {
      console.warn('[Sitemap Risk-4] No tickers returned from getAllTickers()');
      return [];
    }
    ...
    // Split tickers: fourth sixteenth goes to sitemap-risk-1
    const sixteenth = Math.floor(uniqueTickers.length / 16);
    const fourthSixteenth = uniqueTickers.slice(sixteenth * 3, sixteenth * 4);
    
    // Track URLs to prevent duplicates within sitemap
    const seenUrls = new Set<string>();
    
    fourthSixteenth.forEach((ticker) => {
    ...
    if (riskPages.length > MAX_URLS_PER_SITEMAP) {
      console.warn(`[Sitemap Risk-4] WARNING: ${riskPages.length} URLs exceeds Google's 50,000 limit. Truncating to ${MAX_URLS_PER_SITEMAP}.`);
      return riskPages.slice(0, MAX_URLS_PER_SITEMAP);
    }
    
    console.log(`[Sitemap Risk-4] Generated ${riskPages.length} risk pages from ${fourthSixteenth.length} tickers (fourth sixteenth)`);
    return riskPages;
  } catch (error) {
    console.error('[Sitemap Risk-4] Error generating risk pages:', error);
    return [];
  }
}
