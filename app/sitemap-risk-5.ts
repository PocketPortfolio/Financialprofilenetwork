/**
 * Sitemap: Risk Pages (Part 5)
 * fifth sixteenth of risk pages
...
export default async function sitemapRisk5(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    if (!Array.isArray(tickers)) {
      console.error('[Sitemap Risk-5] getAllTickers() did not return an array:', typeof tickers);
      return [];
    }
    
    if (tickers.length === 0) {
      console.warn('[Sitemap Risk-5] No tickers returned from getAllTickers()');
      return [];
    }
    ...
    // Split tickers: fifth sixteenth goes to sitemap-risk-1
    ...
    if (riskPages.length > MAX_URLS_PER_SITEMAP) {
      console.warn(`[Sitemap Risk-5] WARNING: ${riskPages.length} URLs exceeds Google's 50,000 limit. Truncating to ${MAX_URLS_PER_SITEMAP}.`);
      return riskPages.slice(0, MAX_URLS_PER_SITEMAP);
    }
    
    console.log(`[Sitemap Risk-5] Generated ${riskPages.length} risk pages from ${fifthSixteenth.length} tickers (fifth sixteenth)`);
    return riskPages;
  } catch (error) {
    console.error('[Sitemap Risk-5] Error generating risk pages:', error);
    return [];
  }
}
