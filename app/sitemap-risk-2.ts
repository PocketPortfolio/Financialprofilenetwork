/**
 * Sitemap: Risk Pages (Part 2)
 * Second sixteenth of risk pages
...
export default async function sitemapRisk2(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    if (!Array.isArray(tickers)) {
      console.error('[Sitemap Risk-2] getAllTickers() did not return an array:', typeof tickers);
      return [];
    }
    
    if (tickers.length === 0) {
      console.warn('[Sitemap Risk-2] No tickers returned from getAllTickers()');
      return [];
    }
    ...
    // Split tickers: second sixteenth goes to sitemap-risk-2
    const sixteenth = Math.floor(uniqueTickers.length / 16);
    const secondSixteenth = uniqueTickers.slice(sixteenth, sixteenth * 2);
    
    // Track URLs to prevent duplicates within sitemap
    const seenUrls = new Set<string>();
    
    secondSixteenth.forEach((ticker) => {
    ...
    if (riskPages.length > MAX_URLS_PER_SITEMAP) {
      console.warn(`[Sitemap Risk-2] WARNING: ${riskPages.length} URLs exceeds Google's 50,000 limit. Truncating to ${MAX_URLS_PER_SITEMAP}.`);
      return riskPages.slice(0, MAX_URLS_PER_SITEMAP);
    }
    
    console.log(`[Sitemap Risk-2] Generated ${riskPages.length} risk pages from ${secondSixteenth.length} tickers (second sixteenth)`);
    return riskPages;
  } catch (error) {
    console.error('[Sitemap Risk-2] Error generating risk pages:', error);
    return [];
  }
}
