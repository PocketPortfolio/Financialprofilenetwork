/**
 * Sitemap: Risk Pages (Part 1)
 * First sixteenth of risk pages
...
export default async function sitemapRisk1(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    if (!Array.isArray(tickers)) {
      console.error('[Sitemap Risk-1] getAllTickers() did not return an array:', typeof tickers);
      return [];
    }
    
    if (tickers.length === 0) {
      console.warn('[Sitemap Risk-1] No tickers returned from getAllTickers()');
      return [];
    }
    ...
    // Split tickers: first sixteenth goes to sitemap-risk-1
    ...
    if (riskPages.length > MAX_URLS_PER_SITEMAP) {
      console.warn(`[Sitemap Risk-1] WARNING: ${riskPages.length} URLs exceeds Google's 50,000 limit. Truncating to ${MAX_URLS_PER_SITEMAP}.`);
      return riskPages.slice(0, MAX_URLS_PER_SITEMAP);
    }
    
    console.log(`[Sitemap Risk-1] Generated ${riskPages.length} risk pages from ${firstSixteenth.length} tickers (first sixteenth)`);
    return riskPages;
  } catch (error) {
    console.error('[Sitemap Risk-1] Error generating risk pages:', error);
    return [];
  }
}
