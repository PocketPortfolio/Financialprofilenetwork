/**
 * Sitemap: Risk Pages (Part 8)
 * eighth sixteenth of risk pages
...
export default async function sitemapRisk8(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    if (!Array.isArray(tickers)) {
      console.error('[Sitemap Risk-8] getAllTickers() did not return an array:', typeof tickers);
      return [];
    }
    
    if (tickers.length === 0) {
      console.warn('[Sitemap Risk-8] No tickers returned from getAllTickers()');
      return [];
    }
    ...
    // Split tickers: eighth sixteenth goes to sitemap-risk-1
    const sixteenth = Math.floor(uniqueTickers.length / 16);
    const eighthSixteenth = uniqueTickers.slice(sixteenth * 7, sixteenth * 8);
    
    // Track URLs to prevent duplicates within sitemap
    const seenUrls = new Set<string>();
    
    eighthSixteenth.forEach((ticker) => {
    ...
    if (riskPages.length > MAX_URLS_PER_SITEMAP) {
      console.warn(`[Sitemap Risk-8] WARNING: ${riskPages.length} URLs exceeds Google's 50,000 limit. Truncating to ${MAX_URLS_PER_SITEMAP}.`);
      return riskPages.slice(0, MAX_URLS_PER_SITEMAP);
    }
    
    console.log(`[Sitemap Risk-8] Generated ${riskPages.length} risk pages from ${eighthSixteenth.length} tickers (eighth sixteenth)`);
    return riskPages;
  } catch (error) {
    console.error('[Sitemap Risk-8] Error generating risk pages:', error);
    return [];
  }
}
