/**
 * Sitemap: Risk Pages (Part 15)
 * fifteenth sixteenth of risk pages
...
export default async function sitemapRisk15(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    if (!Array.isArray(tickers)) {
      console.error('[Sitemap Risk-15] getAllTickers() did not return an array:', typeof tickers);
      return [];
    }
    
    if (tickers.length === 0) {
      console.warn('[Sitemap Risk-15] No tickers returned from getAllTickers()');
      return [];
    }
    ...
    // Split tickers: fifteenth sixteenth goes to sitemap-risk-1
    ...
    if (riskPages.length > MAX_URLS_PER_SITEMAP) {
      console.warn(`[Sitemap Risk-15] WARNING: ${riskPages.length} URLs exceeds Google's 50,000 limit. Truncating to ${MAX_URLS_PER_SITEMAP}.`);
      return riskPages.slice(0, MAX_URLS_PER_SITEMAP);
    }
    
    console.log(`[Sitemap Risk-15] Generated ${riskPages.length} risk pages from ${fifteenthSixteenth.length} tickers (fifteenth sixteenth)`);
    return riskPages;
  } catch (error) {
    console.error('[Sitemap Risk-15] Error generating risk pages:', error);
    return [];
  }
}
