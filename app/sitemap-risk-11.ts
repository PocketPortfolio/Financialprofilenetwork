/**
 * Sitemap: Risk Pages (Part 11)
 * eleventh sixteenth of risk pages
...
export default async function sitemapRisk11(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    if (!Array.isArray(tickers)) {
      console.error('[Sitemap Risk-11] getAllTickers() did not return an array:', typeof tickers);
      return [];
    }
    
    if (tickers.length === 0) {
      console.warn('[Sitemap Risk-11] No tickers returned from getAllTickers()');
      return [];
    }
    ...
    // Split tickers: eleventh sixteenth goes to sitemap-risk-1
    ...
    if (riskPages.length > MAX_URLS_PER_SITEMAP) {
      console.warn(`[Sitemap Risk-11] WARNING: ${riskPages.length} URLs exceeds Google's 50,000 limit. Truncating to ${MAX_URLS_PER_SITEMAP}.`);
      return riskPages.slice(0, MAX_URLS_PER_SITEMAP);
    }
    
    console.log(`[Sitemap Risk-11] Generated ${riskPages.length} risk pages from ${eleventhSixteenth.length} tickers (eleventh sixteenth)`);
    return riskPages;
  } catch (error) {
    console.error('[Sitemap Risk-11] Error generating risk pages:', error);
    return [];
  }
}
