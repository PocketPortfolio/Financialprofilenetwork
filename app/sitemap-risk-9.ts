/**
 * Sitemap: Risk Pages (Part 9)
 * ninth sixteenth of risk pages
...
export default async function sitemapRisk9(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    if (!Array.isArray(tickers)) {
      console.error('[Sitemap Risk-9] getAllTickers() did not return an array:', typeof tickers);
      return [];
    }
    
    if (tickers.length === 0) {
      console.warn('[Sitemap Risk-9] No tickers returned from getAllTickers()');
      return [];
    }
    ...
    // Split tickers: ninth sixteenth goes to sitemap-risk-1
    const sixteenth = Math.floor(uniqueTickers.length / 16);
    const ninthSixteenth = uniqueTickers.slice(sixteenth * 8, sixteenth * 9);
    
    // Track URLs to prevent duplicates within sitemap
    const seenUrls = new Set<string>();
    
    ninthSixteenth.forEach((ticker) => {
    ...
    if (riskPages.length > MAX_URLS_PER_SITEMAP) {
      console.warn(`[Sitemap Risk-9] WARNING: ${riskPages.length} URLs exceeds Google's 50,000 limit. Truncating to ${MAX_URLS_PER_SITEMAP}.`);
      return riskPages.slice(0, MAX_URLS_PER_SITEMAP);
    }
    
    console.log(`[Sitemap Risk-9] Generated ${riskPages.length} risk pages from ${ninthSixteenth.length} tickers (ninth sixteenth)`);
    return riskPages;
  } catch (error) {
    console.error('[Sitemap Risk-9] Error generating risk pages:', error);
    return [];
  }
}
