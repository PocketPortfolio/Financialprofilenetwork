/**
 * Sitemap: Risk Pages (Part 12)
 * twelfth sixteenth of risk pages
...
export default async function sitemapRisk12(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    if (!Array.isArray(tickers)) {
      console.error('[Sitemap Risk-12] getAllTickers() did not return an array:', typeof tickers);
      return [];
    }
    
    if (tickers.length === 0) {
      console.warn('[Sitemap Risk-12] No tickers returned from getAllTickers()');
      return [];
    }
    ...
    // Split tickers: twelfth sixteenth goes to sitemap-risk-1
    const sixteenth = Math.floor(uniqueTickers.length / 16);
    const twelfthSixteenth = uniqueTickers.slice(sixteenth * 11, sixteenth * 12);
    
    // Track URLs to prevent duplicates within sitemap
    const seenUrls = new Set<string>();
    
    twelfthSixteenth.forEach((ticker) => {
    ...
    if (riskPages.length > MAX_URLS_PER_SITEMAP) {
      console.warn(`[Sitemap Risk-12] WARNING: ${riskPages.length} URLs exceeds Google's 50,000 limit. Truncating to ${MAX_URLS_PER_SITEMAP}.`);
      return riskPages.slice(0, MAX_URLS_PER_SITEMAP);
    }
    
    console.log(`[Sitemap Risk-12] Generated ${riskPages.length} risk pages from ${twelfthSixteenth.length} tickers (twelfth sixteenth)`);
    return riskPages;
  } catch (error) {
    console.error('[Sitemap Risk-12] Error generating risk pages:', error);
    return [];
  }
}
