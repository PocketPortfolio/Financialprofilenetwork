...
export async function GET(request: Request) {
  ...
  const envTestEmail = process.env.STACK_REVEAL_TEST_EMAIL;
  const testMode = verifyCronTestMode(
    searchParams,
    searchParams.get('email') || envTestEmail || undefined,
  );
  if (!testMode.ok) {
    return NextResponse.json({ error: testMode.error }, { status: testMode.status });
  }

  const testEmail = testMode.testEmail || envTestEmail;
  const isTestRun = testMode.isTestRun || !!envTestEmail;
