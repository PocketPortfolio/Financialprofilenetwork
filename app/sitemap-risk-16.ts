/**
 * Sitemap: Risk Pages (Part 16)
 * sixteenth sixteenth of risk pages
...
export default async function sitemapRisk16(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    if (!Array.isArray(tickers)) {
      console.error('[Sitemap Risk-16] getAllTickers() did not return an array:', typeof tickers);
      return [];
    }
    
    if (tickers.length === 0) {
      console.warn('[Sitemap Risk-16] No tickers returned from getAllTickers()');
      return [];
    }
    ...
    // Split tickers: sixteenth sixteenth goes to sitemap-risk-1
    const sixteenth = Math.floor(uniqueTickers.length / 16);
    const sixteenthSixteenth = uniqueTickers.slice(sixteenth * 15);
    
    // Track URLs to prevent duplicates within sitemap
    const seenUrls = new Set<string>();
    
    sixteenthSixteenth.forEach((ticker) => {
    ...
    if (riskPages.length > MAX_URLS_PER_SITEMAP) {
      console.warn(`[Sitemap Risk-16] WARNING: ${riskPages.length} URLs exceeds Google's 50,000 limit. Truncating to ${MAX_URLS_PER_SITEMAP}.`);
      return riskPages.slice(0, MAX_URLS_PER_SITEMAP);
    }
    
    console.log(`[Sitemap Risk-16] Generated ${riskPages.length} risk pages from ${sixteenthSixteenth.length} tickers (sixteenth sixteenth)`);
    return riskPages;
  } catch (error) {
    console.error('[Sitemap Risk-16] Error generating risk pages:', error);
    return [];
  }
}
