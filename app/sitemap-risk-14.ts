/**
 * Sitemap: Risk Pages (Part 14)
 * fourteenth sixteenth of risk pages
...
export default async function sitemapRisk14(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    if (!Array.isArray(tickers)) {
      console.error('[Sitemap Risk-14] getAllTickers() did not return an array:', typeof tickers);
      return [];
    }
    
    if (tickers.length === 0) {
      console.warn('[Sitemap Risk-14] No tickers returned from getAllTickers()');
      return [];
    }
    ...
    // Split tickers: fourteenth sixteenth goes to sitemap-risk-1
    ...
    if (riskPages.length > MAX_URLS_PER_SITEMAP) {
      console.warn(`[Sitemap Risk-14] WARNING: ${riskPages.length} URLs exceeds Google's 50,000 limit. Truncating to ${MAX_URLS_PER_SITEMAP}.`);
      return riskPages.slice(0, MAX_URLS_PER_SITEMAP);
    }
    
    console.log(`[Sitemap Risk-14] Generated ${riskPages.length} risk pages from ${fourteenthSixteenth.length} tickers (fourteenth sixteenth)`);
    return riskPages;
  } catch (error) {
    console.error('[Sitemap Risk-14] Error generating risk pages:', error);
    return [];
  }
}
