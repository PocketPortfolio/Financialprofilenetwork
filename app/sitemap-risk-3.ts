/**
 * Sitemap: Risk Pages (Part 3)
 * third sixteenth of risk pages
...
export default async function sitemapRisk3(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    if (!Array.isArray(tickers)) {
      console.error('[Sitemap Risk-3] getAllTickers() did not return an array:', typeof tickers);
      return [];
    }
    
    if (tickers.length === 0) {
      console.warn('[Sitemap Risk-3] No tickers returned from getAllTickers()');
      return [];
    }
    ...
    // Split tickers: third sixteenth goes to sitemap-risk-1
    const sixteenth = Math.floor(uniqueTickers.length / 16);
    const thirdSixteenth = uniqueTickers.slice(sixteenth * 2, sixteenth * 3);
    
    // Track URLs to prevent duplicates within sitemap
    const seenUrls = new Set<string>();
    
    thirdSixteenth.forEach((ticker) => {
    ...
    if (riskPages.length > MAX_URLS_PER_SITEMAP) {
      console.warn(`[Sitemap Risk-3] WARNING: ${riskPages.length} URLs exceeds Google's 50,000 limit. Truncating to ${MAX_URLS_PER_SITEMAP}.`);
      return riskPages.slice(0, MAX_URLS_PER_SITEMAP);
    }
    
    console.log(`[Sitemap Risk-3] Generated ${riskPages.length} risk pages from ${thirdSixteenth.length} tickers (third sixteenth)`);
    return riskPages;
  } catch (error) {
    console.error('[Sitemap Risk-3] Error generating risk pages:', error);
    return [];
  }
}
