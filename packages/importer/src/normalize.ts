...
export function toISO(value: string, locale: string): string {
  if (!value || !value.trim()) throw new Error(`Empty date value`);
  const v = value.trim();
  ...
  if (m) {
    ...
    if (isNaN(+iso)) throw new Error(`Invalid date: ${value}`);
...
export function toNumber(value: string | number, locale: string): number {
  ...
  if (hasComma && hasDot) {
    ...
    if (cleaned.indexOf(',') < cleaned.indexOf('.')) n = cleaned.replace(/,/g,'');
    else n = cleaned.replace(/\./g,'').replace(',','.');
