...
export const fidelity: BrokerAdapter = {
  id: 'fidelity',
  detect: (sample) => /(^|\n)(Run Date|Symbol|Action|Quantity|Price|Fidelity)/i.test(sample),
  parse: async (file, locale='en-US') => {
    ...
    for (const r of rows) {
      try {
        ...
        const out: NormalizedTrade = {
          date: toISO(r['Run Date'] ?? r['Date'] ?? r['Trade Date'] ?? '', locale),
    ...
    return { broker: 'fidelity', trades, warnings, meta };
  }
};
