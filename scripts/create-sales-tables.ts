...
async function createTables() {
  try {
    ...
    await sql`
      DO $$ BEGIN
        CREATE TYPE lead_status AS ENUM ('NEW', 'RESEARCHING', 'CONTACTED', 'REPLIED', 'INTERESTED', 'NOT_INTERESTED', 'DO_NOT_CONTACT', 'CONVERTED');
      EXCEPTION
    ...
    await sql`
      DO $$ BEGIN
        CREATE TYPE audit_action AS ENUM ('EMAIL_SENT', 'EMAIL_RECEIVED', 'RESEARCH_DONE', 'LEAD_SCORED', 'STATUS_CHANGED', 'COMPLIANCE_CHECK', 'RATE_LIMIT_HIT', 'KILL_SWITCH_ACTIVATED', 'ENRICHMENT_FAILED', 'EMAIL_FAILED', 'AUTONOMOUS_REPLY_SENT', 'AI_REPLY_SENT', 'AUTONOMOUS_REPLY_FAILED');
      EXCEPTION
        WHEN duplicate_object THEN null;
      END $$;
    `;
