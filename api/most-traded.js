...
// [{ rank, symbol, price, changePct, volume, marketCap, currency }, ...]

export const config = { runtime: 'nodejs' }; // Vercel Node runtime

export default async function handler(req, res) {
  // CORS is harmless and can help during local testing
  ...
  try {
    ...
    // Yahoo “Most Actives” predefined screener (server-side safe)
    const url =
    ...
    if (!r.ok) {
      // Don’t throw → UI will show “No data” gracefully
    ...
    // Light caching—fresh enough for a “most traded” panel
    res.setHeader('Cache-Control', 's-maxage=30, stale-while-revalidate=30');
    return res.status(200).json(out);
  } catch (err) {
    // Never surface a 500 to the client; keep the UI calm
    res.setHeader('Cache-Control', 'no-store');
    return res.status(200).json([]);
  }
}
